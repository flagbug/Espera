    ...
    public sealed class YoutubeSongFinder : IYoutubeSongFinder
    {
        ...
        private static IObservable<IReadOnlyList<YoutubeSong>> RealSearch(string searchTerm)
        {
            ...
            return Observable.FromAsync(async () =>
                {
                    var feed = await Task.Run(() => request.Get<Video>(query));
                    var entries = await Task.Run(() => feed.Entries.ToList());

                    var songs = new List<YoutubeSong>();

                    foreach (var video in entries)
                    {
                        var duration = TimeSpan.FromSeconds(int.Parse(video.YouTubeEntry.Duration.Seconds));
                        var url = video.WatchPage.OriginalString
                            .Replace("&feature=youtube_gdata_player", string.Empty) // Unnecessary long url
                            .Replace("https://",
                                "http://"); // Secure connections are not always easy to handle when streaming
                ...
                .Catch<IReadOnlyList<YoutubeSong>, Exception>(ex =>
                    Observable.Throw<IReadOnlyList<YoutubeSong>>(
