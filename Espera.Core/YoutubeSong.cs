...
namespace Espera.Core
{
    public sealed class YoutubeSong : Song
    {
        ...
        public override string PlaybackPath => playbackPath;
        ...
        public static async Task DownloadAudioAsync(VideoInfo videoInfo, string downloadPath,
            IObserver<double> progress)
        {
            var cleanedTitle = RemoveIllegalPathCharacters(videoInfo.Title);
            var downloader = new AudioDownloader(videoInfo,
                Path.Combine(downloadPath, cleanedTitle + videoInfo.AudioExtension));

            downloader.DownloadProgressChanged += (sender, args) => progress.OnNext(args.ProgressPercentage * 0.95);
            downloader.AudioExtractionProgressChanged +=
                (sender, args) => progress.OnNext(95 + args.ProgressPercentage * 0.05);

            await DownloadFromYoutube(downloader,
                new[] { typeof(IOException), typeof(WebException), typeof(AudioExtractionException) }, progress);
        }

        public static async Task DownloadVideoAsync(VideoInfo videoInfo, string downloadPath,
            IObserver<double> progress)
        {
            var cleanedTitle = RemoveIllegalPathCharacters(videoInfo.Title);
            var downloader = new VideoDownloader(videoInfo,
                Path.Combine(downloadPath, cleanedTitle + videoInfo.VideoExtension));
        ...
        internal override async Task PrepareAsync(YoutubeStreamingQuality qualityHint)
        {
            ...
            try
            {
                video = await GetVideoInfoForStreaming(OriginalPath, qualityHint);
            ...
            if (video == null) throw new SongPreparationException("No suitable video found");

            playbackPath = video.DownloadUrl;
        }

        private static async Task DownloadFromYoutube(Downloader downloader, IEnumerable<Type> exceptionTypes,
            IObserver<double> progress)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                if (exceptionTypes.Contains(ex.GetType())) progress.OnError(outer);
            ...
            YoutubeStreamingQuality quality)
        {
            ...
            if (video == null) return GetVideoByStreamingQuality(videos, (YoutubeStreamingQuality)((int)quality - 1));
            ...
            YoutubeStreamingQuality qualitySetting)
        {
            IEnumerable<VideoInfo> videoInfos =
                await Task.Run(() => DownloadUrlResolver.GetDownloadUrls(youtubeLink, false));
